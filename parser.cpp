...
int ParseExpPrim(char*& p, aint& nval) {
	...
	} else if (Options::MemoryType != MT_NONE && *p == '{') {
	...
	} else if (Options::MemoryType != MT_NONE && *p == '$' && *(p + 1) == '$') {
	  	++p; ++p; nval = MemoryCPage; return 1;
	} else if (*p == '$') {
...
char* ReplaceDefine(char* lp) {
	...
	while ('o') {
		...
		if (!(ver = DefineTable.getverv(nid))) {
		...
		/* (begin add) */
		if (DefineTable.defarraylstp) {
			CStringList* a = DefineTable.defarraylstp;
			aint val;
			//cout << lp << endl;
			while (*(lp++) && (*lp <= ' ' || *lp == '['));
			//cout << lp << endl;
	...
	*tp = 0; if (*lp == ':') {
	...
	if (isdigit((unsigned char) * tp)) {
		...
		if (NeedEQU()) {
			...
			aint nv;
			val = AddressOfMAP;
			synerr = 0; if (ParseExpression(lp, nv)) {
							AddressOfMAP += nv;
						} synerr = 1;
			if (IsLabelNotFound) {
				Error("Forward reference", 0, PASS1);
		...
		if (pass == 2) {
			...
			if (!IsDEFL && val != oval) {
	  			...
	  			cout << "(DEBUG) " << filename << " line " << CurrentLocalLine << ": " << "Label has different value in pass 2: ";
	  			cout << val << "!=" << oval << endl;
	  			LabelTable.Update(tp, val);
			}
		} else/*if (!LabelTable.Insert(tp,val,false)) Error("Duplicate label",tp,PASS1);*/ if (!LabelTable.Insert(tp, val, false, IsDEFL)) {
		  	Error("Duplicate label", tp, PASS1);
		}
	}
}
